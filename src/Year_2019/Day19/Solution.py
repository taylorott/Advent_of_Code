...
def solution01():
    ...
    for i in range(0,50):
        for j in range(0,50):
            output_tape = myComp.run_code([i,j],silence_errors=True)
            myComp.reset()
            if output_tape[0]==1:
                total+=1
    print(total)

def trace_boundary(myComp,l0,l1):
    ...
    while i<l0 and j<l1:
        myComp.reset()
        test_val = myComp.run_code([i,j],silence_errors=True)[0]
        while test_val!=1:
            j+=1
            myComp.reset()
            test_val = myComp.run_code([i,j],silence_errors=True)[0]
...
def solution02():
    ...
    bot_dict,top_dict = trace_boundary(myComp,l0,l1)
    ...
    for bot_coords in bot_dict.keys():
        bot_index = bot_coords[0]
        left_index = bot_coords[1]
        top_index = bot_index-height+1
        right_index = left_index+width-1

        if (top_index,right_index) in top_dict:
            temp_dist = left_index+top_index
            if temp_dist<corner_dist:
                corner_dist = temp_dist
                corner_coords = [top_index,left_index]
      
    print(10000*corner_coords[0]+corner_coords[1])
