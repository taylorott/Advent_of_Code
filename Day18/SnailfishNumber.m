classdef SnailfishNumber < handle
    ...
    methods
        ...
        function [bit_code,red_type] = find_leftmost_reduction_target(obj)
            [found_flag,bit_code] = obj.find_leftmost_explode_target();
            if found_flag
                red_type = 1;
            else
                [found_flag,bit_code] = obj.find_leftmost_split_target();
                if found_flag
                    red_type = 2;
                else
                    red_type = 0;
        ...
        function [found_flag,bit_code] = find_leftmost_split_target(obj)
            [found_flag,bit_code] = obj.find_leftmost_explode_split_recursive([],obj.myTree);
        end
        
        %red_type == 0 means no explosion/splitting necessary
        %red_type == 1 means explosion is necessary
        %red_type == 2 means split is necessary
        function [found_flag,bit_code] = find_leftmost_explode_split_recursive(obj,bit_code,inspection_target)
            if isa(inspection_target,'double')
                %if the current element we are looking at is a number....
                found_flag = inspection_target>=10;
            else
                %if the current element we are looking at is a pair...
  
                %first, check the left child/subtree for a split target
                [potential_found_flag, potential_bit_code] = obj.find_leftmost_explode_split_recursive([bit_code,0],inspection_target.left_child);
                ...
                if potential_found_flag
                    ...
                    %otherwise, check the right subtree
                    [found_flag,bit_code] = obj.find_leftmost_explode_split_recursive([bit_code,1],inspection_target.right_child);
                end
