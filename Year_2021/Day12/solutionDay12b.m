...
small_cave_has_repeated = [0];

while length(path_list_current)>0
    path_list_new = {};
    small_cave_has_repeated_new = [];
    ...
    for i = 1:length(path_list_current)
        ...
        for j = 1:length(adjacency_list{last_step})
            ...
            if cave_size(possible_step)==0 && possible_step~=start_index
%                 sum(possible_step==path_list_current{i})
                if ~ismember(possible_step,path_list_current{i})
                    path_list_new{end+1} = [path_list_current{i},possible_step];
                    small_cave_has_repeated_new(end+1) = small_cave_has_repeated(i);
                else
                    if small_cave_has_repeated(i) == 0
                        path_list_new{end+1} = [path_list_current{i},possible_step];
                        small_cave_has_repeated_new(end+1) = 1;
                    end
                end
            end
    ...
    small_cave_has_repeated = small_cave_has_repeated_new;
end
